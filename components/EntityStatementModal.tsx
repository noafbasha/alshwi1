...
const EntityStatementModal: React.FC<Props> = ({ entityId, entityType, onClose }) => {
      ...
      html2canvas: { 
        ...
        onclone: (clonedDoc: Document) => {
          ...
          if (content) {
            ...
            content.querySelectorAll('.text-slate-400').forEach((el: any) => el.style.color = '#64748b');
          }
        }
      },
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-2 md:p-8 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-7xl bg-white dark:bg-slate-900 rounded-[2rem] md:rounded-[3rem] shadow-2xl flex flex-col h-[95vh] md:h-[90vh] overflow-hidden border border-white/10">

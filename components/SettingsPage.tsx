...
const SettingsPage: React.FC = () => {
          ...
          <div className="space-y-8 animate-in slide-in-from-left duration-300">
            <div className="flex flex-col items-center p-8 bg-slate-50 dark:bg-slate-800/50 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-700">
               <div className="relative group">
                  <div className="w-32 h-32 md:w-44 md:h-44 bg-emerald-600 rounded-[2.5rem] flex items-center justify-center text-5xl md:text-7xl text-white shadow-2xl transition-transform group-hover:rotate-6">
                    {profile?.full_name?.[0] || '👤'}
                  </div>
                  <button className="absolute -bottom-2 -right-2 bg-white dark:bg-slate-900 p-3 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 text-xl hover:scale-110 transition-all">📸</button>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <SettingRow label="البريد الإلكتروني" desc="يستخدم لاستعادة الحساب">
                  <input className="setting-input opacity-50 cursor-not-allowed" value={user?.email} disabled />
               </SettingRow>
               <SettingRow label="كلمة المرور" desc="تغيير كلمة المرور الحالية">
                  <button className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-sm hover:bg-black transition-all">تحديث الأمان 🔐</button>
               </SettingRow>
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-8 animate-in slide-in-from-left duration-300">
             <SettingRow label="تفعيل قفل التطبيق" desc="طلب رمز PIN عند فتح التطبيق لحماية الخصوصية">
                <Toggle checked={localSettings.security.appLockEnabled} onChange={v => updateNested('security', 'appLockEnabled', v)} />
             </SettingRow>
             {localSettings.security.appLockEnabled && (
               <>
                ...
                <SettingRow label="قفل تلقائي عند الخروج" desc="تفعيل القفل فور إغلاق المتصفح أو التطبيق">
                   <Toggle checked={localSettings.security.autoLockOnExit} onChange={v => updateNested('security', 'autoLockOnExit', v)} />
                </SettingRow>
               </>
             )}
          </div>
        ...
        return (
          <div className="space-y-8 animate-in slide-in-from-left duration-300">
             <SettingRow label="اسم الوكالة" desc="يظهر في واجهة البرنامج والتقارير المطبوعة">
                <input className="setting-input" value={localSettings.agency.name} onChange={e => updateNested('agency', 'name', e.target.value)} />
             </SettingRow>
             <SettingRow label="رقم التواصل" desc="يستخدم في تذييل الفواتير وكشوف الحساب">
                <input className="setting-input text-left" dir="ltr" value={localSettings.agency.phone} onChange={e => updateNested('agency', 'phone', e.target.value)} />
             </SettingRow>
             <SettingRow label="العنوان" desc="يظهر في رأس الفاتورة الرسمية">
                <input className="setting-input" value={localSettings.agency.address} onChange={e => updateNested('agency', 'address', e.target.value)} />
             </SettingRow>
          </div>
        );
      case 'ai':
        return (
          <div className="space-y-8 animate-in slide-in-from-left duration-300">
             <SettingRow label="لهجة المساعد" desc="اختر اللهجة التي سيتحدث بها المساعد عند المطالبات">
                <select className="setting-input" value={localSettings.ai.dialect} onChange={e => updateNested('ai', 'dialect', e.target.value)}>
             ...
             <SettingRow label="التحليل اليومي" desc="السماح للمساعد بتحليل أداء الوردية تلقائياً">
                <Toggle checked={localSettings.ai.autoAnalyzeDaily} onChange={v => updateNested('ai', 'autoAnalyzeDaily', v)} />
             </SettingRow>
          </div>
        );
      case 'automation':
          ...
          <div className="space-y-8 animate-in slide-in-from-left duration-300">
             <div className="p-8 bg-orange-50 dark:bg-orange-900/10 rounded-[2rem] border-2 border-orange-100 dark:border-orange-900/30">
                ...
                <SettingRow label="تفعيل التذكير بالديون" desc="إرسال إشعار للمدير لمتابعة العملاء المتأخرين">
                   <Toggle checked={localSettings.debts.autoReminderEnabled} onChange={v => updateNested('debts', 'autoReminderEnabled', v)} />
                </SettingRow>
                <SettingRow label="حد المديونية للتنبيه" desc="نبهني إذا تجاوز دين أي عميل هذا المبلغ">
                   <input type="number" className="setting-input max-w-[200px]" value={localSettings.debts.autoReminderThreshold} onChange={e => updateNested('debts', 'autoReminderThreshold', parseInt(e.target.value))} />
                </SettingRow>
             </div>
          </div>
        );
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                   ...
                   <SettingRow label="ترويسة الفاتورة" desc="نص يظهر في أعلى الفواتير">
                      <input className="setting-input" value={localSettings.agency.headerText} onChange={e => updateNested('agency', 'headerText', e.target.value)} />
                   </SettingRow>
                   <SettingRow label="تذييل الفاتورة" desc="نص ختامي (مثل: البضاعة لا ترد بعد 24 ساعة)">
                      <input className="setting-input" value={localSettings.agency.footerText} onChange={e => updateNested('agency', 'footerText', e.target.value)} />
                   </SettingRow>
                </div>
             ...
             <div className="bg-slate-900 rounded-[3rem] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10">
                   <div className="flex items-center gap-6 mb-8">
                      <div className="w-20 h-20 bg-emerald-600 rounded-[2rem] flex items-center justify-center text-4xl shadow-2xl animate-glow">🛡️</div>
                      <div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <button onClick={exportData} className="bg-white/10 hover:bg-white/20 text-white py-5 rounded-2xl font-black text-xl border border-white/10 transition-all active:scale-95">📤 تصدير نسخة للجهاز</button>
                      <button onClick={() => resetSystem()} className="bg-rose-600 hover:bg-rose-700 text-white py-5 rounded-2xl font-black text-xl transition-all shadow-xl active:scale-95">🚨 تصفير النظام</button>
                   </div>
                </div>
             </div>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto pb-40 animate-in fade-in duration-500 px-2 md:px-0">
        ...
        <aside className="w-full lg:w-80 shrink-0">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden sticky top-32 z-20 transition-all">
             <div className="p-10 border-b dark:border-slate-800 text-center bg-slate-50/50 dark:bg-slate-800/30">
                <div className="w-20 h-20 bg-indigo-600 rounded-[1.8rem] flex items-center justify-center text-4xl text-white shadow-2xl mx-auto mb-4 animate-float">⚙️</div>
             ...
             <nav className="p-4 flex lg:flex-col overflow-x-auto no-scrollbar gap-2">
                {sections.map(s => (
                  <button 
                    key={s.id}
                    onClick={() => setActiveSection(s.id)}
                    className={`whitespace-nowrap flex items-center gap-5 px-6 py-5 rounded-[1.5rem] font-black text-sm transition-all relative group ${activeSection === s.id ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                  >
                    <span className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl transition-all ${activeSection === s.id ? 'bg-white/20' : s.color + ' text-white'}`}>{s.icon}</span>
                    <span>{s.label}</span>
                  </button>
                ))}
             </nav>
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[3rem] md:rounded-[5rem] shadow-2xl border-2 border-slate-100 dark:border-slate-800 overflow-hidden min-h-[800px] flex flex-col transition-all relative">
              ...
              <div className="p-8 md:p-16 border-b border-slate-50 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-8 bg-slate-50/20 dark:bg-slate-800/10">
                 <div className="flex items-center gap-6 md:gap-8">
                    <div className="w-20 h-20 md:w-28 md:h-28 bg-white dark:bg-slate-800 rounded-[2.5rem] md:rounded-[3.5rem] flex items-center justify-center text-4xl md:text-6xl shadow-2xl border-2 border-slate-100 dark:border-slate-700 animate-in zoom-in duration-500">
                 ...
                 {activeSection !== 'integrity' && activeSection !== 'backup' && (
                    <button onClick={handleSave} className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-12 py-5 rounded-[2rem] font-black text-xl md:text-2xl shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-4">
      ...
      <style>{`
        .setting-input {
          @apply w-full p-5 bg-slate-50 dark:bg-slate-950 border-4 border-transparent rounded-[1.5rem] outline-none font-black text-xl text-slate-900 dark:text-white transition-all shadow-inner focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 focus:shadow-2xl;
        }
      `}</style>
    </div>
  );
...
const Toggle = ({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) => (
  <button onClick={() => onChange(!checked)} className={`w-24 h-12 rounded-full transition-all relative flex items-center px-2 ${checked ? 'bg-emerald-500 shadow-inner' : 'bg-slate-300 dark:bg-slate-700'}`}>
